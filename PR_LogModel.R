...
notfts<-c('target',"replacement_code","X","rptd_dt","iss_dt",'sum_entrydate','fn','ln','ssn','status','polnum','final_date')
model=cv.glmnet(x=d[,!(names(d) %in% notfts)], d$target, family='binomial', alpha=0)
save(file="Full_GLM_With_EZApp.Rda",model)
