...
#We assume everyone will report - must determine how many days until they do report.  
d<-read.csv("PolicyDataFromJan2014.csv")


d$marital[d$marital=='']='U'
d$sum_entrydate<-as.POSIXct(d$sum_entrydate)
d$final_date<-as.POSIXct(d$final_date)
d<-filter(d,year(final_date)<9000)

d$target=as.numeric(difftime(d$final_date,d$sum_entrydate,units="weeks"))
...
notfts<-c("replacement_code","X","rptd_dt","iss_dt",'sum_entrydate','fn','ln','ssn','status','polnum','eff_date','policy_status','insured_id','target')
model=randomForest(d[,!(names(d) %in% notfts)], d$target, n.tree=500)
save(file='PT_RF_EZApp.Rda',model)
